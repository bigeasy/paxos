describe('paxos', () => {
    ...
    it('can paxos', () => {
        function okay (...vargs) {
            if (vargs.length == 2) {
                console.log('assert')
                assert(vargs[0], vargs[1])
            } else if (typeof vargs[1] == 'object') {
                console.log('assert.deepStrictEqual')
                assert.deepStrictEqual(vargs[0], vargs[1], vargs[2])
            } else {
                console.log('assert.equal')
                assert.equal(vargs[0], vargs[1], vargs[2])
            }
        }
        ...
        assert(network.denizens[0].embark(network.time, 1, '1', network.denizens[1].cookie, { location: '1' }, true).enqueued, 'arrive')

        network.send()

        assert.deepStrictEqual(network.denizens[1].government, {
            republic: 1,
            majority: [ '0' ],
            minority: [],
            acclimated: [ '0', '1' ],
        ...
        assert.deepStrictEqual(network.denizens[2].government, {
            republic: 1,
            majority: [ '0' ],
            minority: [],
            acclimated: [ '0', '1' ],
        ...
        assert(!network.denizens[0].embark(network.time, 1, '1', network.denizens[1].cookie, { location: '1' }).enqueued, 'already arrived')
        assert(!network.denizens[1].enqueue(network.time, 1, {}).enqueued, 'enqueue not leader')

        assert(!network.denizens[1].embark(network.time, 1, '4', 0, { location: '4' }).enqueued, 'arrive not leader')
    ...
    // network.send(1, '0', [ '1' ])
        ...
        assert(!network.denizens[0].enqueue(network.time, 1, {}).enqueued, 'post collapsed')
        ...
        assert.deepStrictEqual(network.denizens[3].government, {
            republic: 1,
            promise: '1a/0',
            majority: [ '3', '0', '2' ],
            minority: [ '6', '7' ],
        ...
        assert.deepStrictEqual(network.denizens[3]._minimums, {
        ...
        assert.deepStrictEqual(network.denizens[6]._disappeared, {}, 'disappeared cleared')

        assert.deepStrictEqual(network.denizens[12].government, {
            republic: 1,
            promise: '23/0',
            majority: [ '3', '0', '2' ],
            minority: [ '6', '7' ],
        ...
        assert.equal(network.denizens[2].government.arrived.promise['13'], '2a/0', 'arrived')
        ...
        assert(!network.denizens[2].government.arrived.promise['13'], 'bad embark cookie')
    })
})
